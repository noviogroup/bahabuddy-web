...
export default async function ConciergeOrderPage({ params, searchParams }: { params: Params; searchParams?: SearchParams }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-5 md:px-6 py-6 pb-10">
      ...
      {order.delivered_plan_url || order.final_itinerary ? <div className="mb-6 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm"><h2 className="mb-3 text-xl font-bold text-night">Delivered plan</h2>{order.delivered_plan_url && <Link href={order.delivered_plan_url} className="inline-flex items-center gap-2 rounded-full bg-brand-600 px-5 py-2.5 font-bold text-white hover:bg-brand-700" target="_blank">Open delivered plan</Link>}{order.final_itinerary && <p className="mt-4 whitespace-pre-wrap text-sm leading-relaxed text-charcoal">{order.final_itinerary}</p>}</div> : <div className="mb-6 rounded-3xl border border-gray-200 bg-gray-50 p-6 text-charcoal"><h2 className="mb-2 text-lg font-bold text-night">Next step</h2><p className="text-sm leading-relaxed">Submit your travel dates, group size, budget, and preferred islands so we can begin preparing your plan.</p></div>}

      <div className="flex flex-col gap-3 sm:flex-row"><Link href={`/dashboard/concierge/${order.id}/details`} className="inline-flex items-center justify-center gap-2 rounded-full bg-brand-600 px-6 py-3 font-bold text-white hover:bg-brand-700">Submit trip details</Link><Link href={createRelatedTripHref} className="inline-flex items-center justify-center rounded-full border border-gray-300 bg-white px-6 py-3 font-bold text-night hover:border-gray-400 hover:bg-gray-50">Create related trip</Link></div>
    </div>
  )
}
