  ...
  return (
    <div className="max-w-3xl mx-auto px-5 md:px-6 py-6 pb-10">
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><p className="text-xs font-bold uppercase text-charcoal">Amount paid</p><p className="mt-2 text-2xl font-bold text-night">{money(order.price_usd)}</p><div className="mt-3 flex flex-wrap gap-2"><Link href={`/dashboard/receipts/${order.id}`} className="inline-flex items-center gap-2 rounded-full bg-brand-600 px-4 py-2 text-xs font-bold text-white hover:bg-brand-700"><span className="h-1.5 w-1.5 rounded-full bg-gold-400" aria-hidden="true" />View receipt</Link><Link href="/dashboard/payments" className="inline-flex rounded-full border border-gray-300 bg-white px-4 py-2 text-xs font-bold text-night hover:border-gray-400 hover:bg-gray-50">Payments</Link></div></div>
      </div>
