        ...
        <div className="divide-y divide-gray-100">
            ...
            <div key={order.id} className="p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="sm:text-right">
                ...
                <div className="mt-2 flex flex-wrap gap-2 sm:justify-end">
                  ...
                  <Link href={`/dashboard/receipts/${order.id}`} className="inline-flex items-center gap-2 rounded-full bg-brand-600 px-4 py-2 text-sm font-bold text-white hover:bg-brand-700">Receipt</Link>
                </div>
              </div>
            </div>
          ))}
          {paidOrders.length === 0 && (
            <div className="p-8 text-center text-charcoal">
              ...
              <Link href="/concierge-trip-plan" className="mt-4 inline-flex items-center gap-2 rounded-full bg-brand-600 px-5 py-2.5 font-bold text-white hover:bg-brand-700">View Concierge offers</Link>
            </div>
          )}
        </div>
      </div>
