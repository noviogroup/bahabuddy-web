    ...
    <div className="max-w-3xl mx-auto px-5 md:px-6 py-6 pb-10">
      ...
      <div className="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-200 bg-gray-50 p-6 md:p-8">
          <p className="inline-flex rounded-full bg-white px-3 py-1 text-xs font-bold uppercase tracking-wide text-charcoal ring-1 ring-gray-200">Receipt</p>
          <h1 className="mt-4 text-3xl font-extrabold tracking-tight text-night md:text-4xl">{receiptNumber}</h1>
          <p className="mt-2 text-charcoal">Baha Buddy Concierge Payment</p>
        </div>

        <div className="p-6 md:p-8">
          <div className="grid sm:grid-cols-2 gap-5 mb-8">
            <div><p className="text-xs font-bold uppercase tracking-wide text-charcoal">Billed to</p><p className="mt-2 font-extrabold text-night">{order.traveler_name || user.email}</p><p className="text-sm text-charcoal">{order.traveler_email || user.email}</p></div>
            <div className="sm:text-right"><p className="text-xs font-bold uppercase tracking-wide text-charcoal">Payment status</p><p className={`mt-2 text-xl font-extrabold capitalize ${paid ? 'text-green-700' : 'text-night'}`}>{label(order.payment_status)}</p><p className="text-sm text-charcoal">{new Date(order.created_at).toLocaleString()}</p></div>
          </div>

          <div className="mb-8 overflow-hidden rounded-2xl border border-gray-200"><div className="grid grid-cols-[1fr_auto] gap-4 bg-gray-50 px-4 py-3 text-xs font-bold uppercase tracking-wide text-charcoal"><span>Description</span><span>Amount</span></div><div className="grid grid-cols-[1fr_auto] gap-4 px-4 py-5 text-sm"><div><p className="font-extrabold text-night capitalize">{label(order.offer_type)}</p><p className="mt-1 text-charcoal">Baha Buddy Concierge planning service</p></div><p className="font-extrabold text-night">{money(order.price_usd)}</p></div><div className="grid grid-cols-[1fr_auto] gap-4 border-t border-gray-200 px-4 py-4"><p className="font-extrabold text-night">Total paid</p><p className="font-extrabold text-night">{paid ? money(order.price_usd) : money(0)}</p></div></div>
