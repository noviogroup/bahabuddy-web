...
export default async function ReceiptPage({ params }: { params: Params }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-5 md:px-6 py-6 pb-10">
      ...
      <div className="mt-5 flex flex-col gap-3 print:hidden sm:flex-row"><Link href={`/dashboard/concierge/${order.id}`} className="inline-flex items-center justify-center gap-2 rounded-full bg-brand-600 px-6 py-3 font-bold text-white hover:bg-brand-700">View order</Link><Link href="/dashboard/payments" className="inline-flex items-center justify-center rounded-full border border-gray-300 bg-white px-6 py-3 font-bold text-night hover:border-gray-400 hover:bg-gray-50">Payment history</Link></div>
    </div>
  )
}
