/**
 * /api/place-photo — server-side proxy for the Google Places Photo API.
 *
 * Why this route exists:
 *   - Google's Photo API requires `key=<api_key>` in the URL. We don't
 *     want that key in `<img src>` rendered to the browser.
 *   - We also want stable cache headers, so the same place photo
 *     doesn't burn Google API quota on every view.
 *
 * Resolution path (companion to `src/lib/place-photos.ts`):
 *   - Client renders `<Image src="/api/place-photo?ref=...&w=...">`
 *   - This route fetches the photo from Google with the server-side key
 *   - Returns the binary with `Cache-Control: public, max-age=31536000,
 *     immutable` so the CDN + browser cache aggressively (photos for a
 *     given `reference` never change — that's how Google's API works)
 *
 * Graceful degradation:
 *   - If `GOOGLE_MAPS_API_KEY` isn't set, return 503 + a tiny SVG
 *     placeholder so the `<Image>` shows a brand-colored tile instead
 *     of a broken icon. The next preferred path is to populate
 *     `google_place_photos.storage_url` via the sync job and never
 *     hit this route at all.
 *
 * Security:
 *   - Validates `ref` looks like a Google photo_reference (alphanumeric
 *     + a small charset) so the route can't be abused to fetch
 *     arbitrary URLs through us.
...
// Google photo references are URL-safe and bounded; this regex is a
...
export async function GET(request: Request): Promise<Response> {
  ...
  // Google Places Photo API redirects (302) to the actual image URL on
  // the Google CDN. We follow the redirect server-side and stream the
  // result back to the client, so the API key never leaves the server.
  ...
  if (!upstream.ok || !upstream.body) {
    return placeholderResponse(upstream.status === 404 ? 404 : 502, `upstream-${upstream.status}`)
  }
