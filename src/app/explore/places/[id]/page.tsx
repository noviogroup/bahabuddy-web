...
async function getSimilarPlaces(place: PlaceDetail): Promise<SimilarPlace[]> {
  const places = await getRelatedExplorePlaces(place, 4)
  return places.map((candidate) => ({
    id: candidate.id,
    name: candidate.name,
...
function displayIslandName(island?: string | null): string | null {
  if (!island) return null
  return getIslandConfig(island)?.name ?? formatLabel(island)
}
...
function islandDetailHref(island?: string | null): string | null {
  if (!island) return null
  const config = getIslandConfig(island)
  return config ? `/explore/island/${config.slug}` : `/explore/places${paramsFrom({ island })}`
}
...
export default async function PlaceDetailPage({ params }: PageProps) {
  ...
  const sourceRecordId = place.source_id ?? place.id
  const mediaPlaceId = place.source_type === 'canonical' ? place.id : sourceRecordId
  const detailPath = place.detail_href ?? `/explore/places/${encodeURIComponent(params.id)}`
  const [photos, reviews, similar] = await Promise.all([
    getPlacePhotos(mediaPlaceId),
    getPlaceReviews(mediaPlaceId),
  ...
  const planPrompt = `Tell me everything about ${place.name}${islandLabel ? ` in ${islandLabel}` : ''}, Bahamas`
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <CompactPageHeader
        ...
        title={place.name}
        subtitle={islandLabel ? `${islandLabel}, Bahamas` : 'Bahamas explore guide'}
        crumbs={[
          { href: '/', label: 'Home' },
      ...
      >
        <div className="flex flex-wrap items-center gap-2">
          ...
          {hasRating && (
            <span className="rounded-full border border-gray-200 bg-white px-3 py-1 text-xs font-bold text-charcoal">
              Rating {rating.toFixed(1)}{place.review_count != null && place.review_count > 0 ? ` (${place.review_count} reviews)` : ''}
            </span>
          )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <aside className="space-y-6">
            <DirectTripItemActions
              ...
              name={place.name}
              island={islandLabel ?? place.island}
        ...
        {similar.length > 0 && (
          <section className="mt-14">
            <h2 className="text-xl font-bold text-gray-900 mb-5">
              More in {islandLabel ?? 'the Bahamas'}
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {similar.map((s) => (
                <Link key={s.id} href={s.detail_href ?? `/explore/places/${s.id}`} className="group">
                  <div className="bg-white rounded-2xl overflow-hidden border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                    <ImageWithSourcePolicy
                      src={s.image_url}
                      alt={s.name}
                      title={s.name}
                      eyebrow={formatLabel(s.category)}
                    ...
                    <div className="p-3">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs text-gray-400">{formatLabel(s.category)}</span>
                        {s.rating && (
