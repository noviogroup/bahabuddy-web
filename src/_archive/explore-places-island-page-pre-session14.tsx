/**
 * /explore/places/[island] — the canonical island detail page.
 *
 * This is where DestinationCard in chat lands (per the chat-vs-detail
 * page split, decision §26). It's a marketing surface (outside the
 * dashboard route group, by intent — decision §2) so it can be
 * crawled, indexed, and shared without auth.
 *
 * Data sources, in priority order:
 *
 *   1. Sanity `destination` document (matched on `islandId === slug`)
 *      provides editorial overview (Portable Text), tagline,
 *      highlights, gallery, bestTimeToVisit, gettingThere, hero image.
 *      Editors curate per island in Studio.
 *
 *   2. Hardcoded `ISLAND_CONFIGS` (in `src/lib/island-config.ts`)
 *      provides fallback values for tagline, hero image, vibe,
 *      bestTime, tripLength, and description. Used when Sanity is
 *      unconfigured, when a specific island has no Sanity document
 *      yet, or as a per-field fallback when Sanity has the
 *      destination but a particular field is blank.
 *
 *   3. Supabase `bahamas_attractions` provides categorized "Things
 *      to Do" — same data the legacy `/destinations/[island]` route
 *      surfaces.
 *
 *   4. Supabase `bahamas_deals` provides current limited-time offers.
 *
 * Static generation: `generateStaticParams` emits the 10 hardcoded
 * ISLAND_CONFIGS slugs at build time. Additional Sanity-only slugs
 * resolve on-demand at request time (dynamicParams defaults to true)
 * and cache for `revalidate` seconds.
 *
 * Revalidation: 300 seconds (5 minutes) so newly-published Sanity
 * content surfaces quickly without a redeploy.
 */
...
// ─── Visual maps (mirrors /destinations/[island]) ───────────────────────────
...
/**
 * Pre-build the 10 hardcoded islands. Sanity-only islands resolve
 * on-demand (Next.js default `dynamicParams: true`).
 */
export async function generateStaticParams() {
  return ISLAND_CONFIGS.map(i => ({ island: i.slug }))
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const config = getIslandConfig(params.island)
  const sanity = await fetchDestinationByIsland(params.island)

  // Nothing matches → don't synthesize metadata for a soon-to-404 page
  if (!config && !sanity) return {}

  const name = sanity?.name ?? config?.name ?? params.island
  ...
  return {
    ...
    alternates: {
      canonical: `/explore/places/${params.island}`,
    },
    openGraph: {
...
export default async function IslandDetailPage({ params }: PageProps) {
  const config = getIslandConfig(params.island)
  // dbSlug is needed for the Supabase queries. For known islands the
  // config carries it (some islands share a dbSlug). For Sanity-only
  // islands without a config, fall back to the URL slug — graceful
  // when no `bahamas_attractions` records exist under that slug.
  const dbSlug = config?.dbSlug ?? params.island

  // Sanity + Supabase fetches in parallel — saves a round-trip.
  const [sanity, attractions, deals] = await Promise.all([
    fetchDestinationByIsland(params.island),
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-6xl mx-auto px-4 py-10">
        ...
        {/* Plan with Buddy CTA (shared component, used across detail pages) */}
        <PlanWithBuddyCTA
          kind="experience"
        ...
        <div className="mt-14">
          ...
          <div className="flex flex-wrap gap-3">
            {ISLAND_CONFIGS.filter(i => i.slug !== params.island).map(island => (
              <Link
                key={island.slug}
                href={`/explore/places/${island.slug}`}
...
function AttractionCard({
  ...
  const learnMoreUrl = `/dashboard/chat?q=${encodeURIComponent(`Tell me about ${attraction.name} in ${islandName}, Bahamas`)}`
