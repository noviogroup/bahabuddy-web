...
export default function GuidedDayTimeline({ stops }: GuidedDayTimelineProps) {
  return (
    <section className="rounded-baha-xl border border-sand-200 bg-white p-6 shadow-card">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-sm font-bold uppercase tracking-wide text-brand-700">Timeline</p>
      ...
      <div className="mt-6 space-y-4">
        {stops.map((stop) => (
          <article key={stop.id} className="rounded-baha-lg border border-sand-200 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="shrink-0 rounded-baha-md bg-sand-50 px-3 py-2 text-sm font-bold text-charcoal">
                {stop.suggested_duration_minutes} min
              </div>
            </div>

            {stop.baha_tip && (
              <div className="mt-4 rounded-baha-md bg-brand-50 p-3 text-sm text-brand-800">
