    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:py-10">
      ...
      {!hasQuery && (
        <section className="py-12 text-center">
          ...
          <h2 className="mt-5 text-2xl font-bold text-night">Search the islands in one place</h2>
          <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-charcoal">
            Find trusted places, beaches, food, stays, tours, deals, and island guides from the Baha Buddy catalog.
          </p>
      ...
      {!loading && !error && results.length > 0 && (
        <section className="mt-8" aria-labelledby="catalog-results-heading">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {results.map((result) => <CatalogResultCard key={`${result.type}-${result.id}`} result={result} />)}
          </div>
        </section>
      )}
    </div>
