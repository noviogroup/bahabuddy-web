...
function DecisionSection({ image }: { image: string }) {
  return (
    <section className="bg-brand-50/55 py-16 sm:py-20" aria-labelledby="buddy-planning-title">
      <div className="mx-auto max-w-6xl px-4">
        <div className="mb-9 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <SectionIntro
            titleId="buddy-planning-title"
            label="Chat with Buddy"
            title="Plan your Bahamas trip in one conversation."
            body="Tell Buddy what matters. He turns it into one connected plan for where to stay, how to get there, and what to do."
          />
        ...
        <div className="grid overflow-hidden rounded-baha-xl border border-brand-100 bg-white shadow-card lg:grid-cols-[0.82fr_1.18fr]">
          <div className="flex flex-col bg-night px-5 py-7 text-white sm:px-8 sm:py-9 lg:min-h-[29rem] lg:px-10 lg:py-11">
            <p className="text-xs font-black uppercase text-brand-200">
              A conversation with Buddy
            </p>

            <div className="mt-7 space-y-3" aria-label="Buddy chat example">
              <div className="ml-auto max-w-[19rem] rounded-baha-lg rounded-tr-sm bg-brand-600 px-4 py-3.5 text-sm font-bold leading-6 text-white">
                Five nights in Nassau for two. Good food, history, and one quiet beach day.
              </div>
              <div className="max-w-[22rem] rounded-baha-lg rounded-tl-sm bg-white px-4 py-3.5 text-sm font-bold leading-6 text-night shadow-soft">
                I’ll shape the island plan, compare stays and flights, then organize each day around what matters to you.
              </div>
            </div>

            <div className="mt-auto border-t border-white/15 pt-5">
              <p className="text-sm font-black text-white">One plan that updates with you</p>
              <p className="mt-1 max-w-sm text-xs font-semibold leading-5 text-white/65">
                Stays, flights, transport, dining, and activities stay connected as your details change.
              </p>
            </div>
          </div>

          <figure className="relative min-h-[21rem] overflow-hidden sm:min-h-[27rem] lg:min-h-[29rem]">
            <Image
              src={image}
              alt="Travelers exploring the Queen's Staircase in Nassau"
              fill
              className="object-cover object-center"
              sizes="(max-width: 1024px) calc(100vw - 2rem), 635px"
              unoptimized
            />
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-night/80 via-night/20 to-transparent px-5 pb-5 pt-16 sm:px-7 sm:pb-7">
              <figcaption className="text-xs font-bold uppercase text-white/80">
                Queen&apos;s Staircase, Nassau · Tourism partner imagery
              </figcaption>
            </div>
          </figure>
        </div>
...
function IslandFitSection({ destinationImages }: { destinationImages: Record<string, string> }) {
  return (
    <section
      className="bg-white py-16 sm:py-20"
        ...
        <div className="-mx-4 overflow-x-auto px-4 pb-3">
          <div className="grid auto-cols-[78vw] grid-flow-col gap-4 sm:auto-cols-[18rem] lg:auto-cols-[16rem]">
            {ISLAND_FIT.map((island) => (
              ...
              >
                <Image
                  src={destinationImages[island.href.split('/').pop() ?? ''] ?? island.image}
