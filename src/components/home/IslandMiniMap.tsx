            ...
            {islands.slice(0, 8).map((island) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`h-2.5 w-2.5 shrink-0 rounded-full ${active ? 'bg-brand-600' : 'bg-gold-400'}`} aria-hidden="true" />
                </button>
              )
            })}
          </div>
...
function renderIslandPins({
  ...
  class IslandPinOverlay extends gmaps.OverlayView {
    ...
    onAdd() {
      ...
      button.innerHTML = `
        <span class="flex h-6 w-6 items-center justify-center rounded-full bg-brand-50 text-brand-700" aria-hidden="true">
